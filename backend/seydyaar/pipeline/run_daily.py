...
from ..models.scoring import HabitatInputs, habitat_scoring, gradient_magnitude, front_score, boa_front_score, eddy_score
...
def _synthetic_env_layers(grid: GridSpec, ts_iso: str) -> Dict[str, np.ndarray]:
    ...
    u = 0.25 * np.sin((lon2d - lon2d.mean()) * math.pi / 10.0) + rng.normal(0, 0.02, size=lon2d.shape)
    v = 0.18 * np.cos((lat2d - lat2d.mean()) * math.pi / 12.0) + rng.normal(0, 0.02, size=lat2d.shape)
    cur = np.sqrt(u*u + v*v)
    ...
    salinity = 35.05 + 0.15 * np.sin((lon2d - lon2d.mean()) * math.pi / 18.0) + rng.normal(0, 0.03, size=chl.shape)
    oxygen = 202.0 + 2.0 * np.cos((lat2d - lat2d.mean()) * math.pi / 16.0) + rng.normal(0, 0.3, size=chl.shape)
    mld = 92.0 + 18.0 * np.sin((lat2d - lat2d.mean()) * math.pi / 20.0) + rng.normal(0, 2.0, size=chl.shape)
    ...
    return {
        ...
        "oxygen_mmol_m3": oxygen.astype(np.float32),
...
def _try_copernicus_layers(
    ...
    try:
        ...
        # Optional enhanced predictors
        for extra_key, out_key in (("salinity", "salinity_psu"), ("oxygen", "oxygen_mmol_m3"), ("mld", "mld_m")):
            cfg = datasets_cfg.get(extra_key, {}) if isinstance(datasets_cfg, dict) else {}
            if not str(cfg.get("dataset_id", "")).strip():
                continue
            try:
                p = _subset_one(extra_key)
                out[out_key] = _to_grid(_read_nc_var(p, _v0(extra_key)))
            except Exception as ee:
                status.setdefault("warnings", []).append(f"optional layer {extra_key} failed: {ee}")
...
def run_daily(
    ...
    for sp, prof in species_profiles.items():
        ...
        sp_meta = {
            ...
            "paths": {
                ...
                "per_time": {
                    ...
                    "pcatch_boaeddy": f"variants/{variant}/species/{sp}/times/{{time}}/pcatch_boaeddy_f32.bin",
                    "phab_scoring": f"variants/{variant}/species/{sp}/times/{{time}}/phab_f32.bin",
                    "phab_boaeddy": f"variants/{variant}/species/{sp}/times/{{time}}/phab_boaeddy_f32.bin",
                    ...
                    "salinity": f"variants/{variant}/species/{sp}/times/{{time}}/salinity_f32.bin",
                    "oxygen": f"variants/{variant}/species/{sp}/times/{{time}}/oxygen_f32.bin",
                    "mld": f"variants/{variant}/species/{sp}/times/{{time}}/mld_f32.bin",
                    "eddy": f"variants/{variant}/species/{sp}/times/{{time}}/eddy_f32.bin",
                    "boa": f"variants/{variant}/species/{sp}/times/{{time}}/boa_f32.bin",
        ...
        for ts_iso in ts_list:
            ...
            boa = boa_front_score(
                layers["sst_c"], np.log10(np.clip(layers["chl_mg_m3"], 1e-6, None)), layers["ssh_m"],
                w_temp=float(priors.get("front_weights", {}).get("temp", 0.35)),
                w_chl=float(priors.get("front_weights", {}).get("chl", 0.45)),
                w_ssh=float(priors.get("front_weights", {}).get("ssh", 0.20)),
            ).astype(np.float32)
            eddy = eddy_score(layers["ssh_m"], layers.get("u_current_m_s"), layers.get("v_current_m_s")).astype(np.float32)
            ...
            inputs = HabitatInputs(
                ...
                ssh_m=layers["ssh_m"],
                salinity_psu=layers.get("salinity_psu"),
                oxygen_mmol_m3=layers.get("oxygen_mmol_m3"),
            ...
            phab_boaeddy = comps.get("phab_boaeddy", phab).astype(np.float32)
            ...
            boaeddy_pcatch = np.clip(phab_boaeddy * pops * (0.85 + 0.15 * boa), 0, 1).astype(np.float32)
            ens = np.nanmean(np.stack([pcatch, m2, boaeddy_pcatch], axis=0), axis=0).astype(np.float32)
            agree, spread = ensemble_stats([pcatch, m2, boaeddy_pcatch])
            ...
            write_bin_f32(tdir / "pcatch_boaeddy_f32.bin", boaeddy_pcatch)
            write_bin_f32(tdir / "phab_f32.bin", phab)
            write_bin_f32(tdir / "phab_boaeddy_f32.bin", phab_boaeddy)
            ...
            write_bin_f32(tdir / "boa_f32.bin", boa)
            write_bin_f32(tdir / "eddy_f32.bin", eddy)
            ...
            if inputs.salinity_psu is not None: write_bin_f32(tdir / "salinity_f32.bin", inputs.salinity_psu.astype(np.float32))
            if inputs.oxygen_mmol_m3 is not None: write_bin_f32(tdir / "oxygen_f32.bin", inputs.oxygen_mmol_m3.astype(np.float32))
            if inputs.mld_m is not None: write_bin_f32(tdir / "mld_f32.bin", inputs.mld_m.astype(np.float32))
    ...
    run_entry = {
        ...
        "variants": [variant],
        "species": list(species_profiles.keys()),
        "models": ["scoring", "frontplus", "boaeddy", "ensemble"],
        "generated_at_utc": now_utc.isoformat().replace("+00:00", "Z"),
    }
