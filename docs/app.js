...
const state = {
  ...
  model: localStorage.getItem("model") || "boaeddy",
...
function initMap(){
  ...
  // 🗺️ Offline basemap (no external tiles). The raster overlays and vectors remain fully interactive.
  const offlineOcean = L.rectangle([[-90,-180],[90,180]], {
    stroke:false,
    fill:true,
    fillColor:"#06253b",
    fillOpacity:1,
  ...
  try{
    fetch("aoi.geojson", {cache:"no-store"})
      .then(r=>r.ok ? r.json() : null)
      .then(gj=>{ if(gj) L.geoJSON(gj, {style:{color:"#9fe7ff", weight:1.4, fill:false, opacity:0.9, interactive:false}}).addTo(map); })
      .catch(()=>{});
  }catch(_){ }
...
async function showPointPopup(lat, lon, metaInfo){
  try{
    ...
    if(singleMode && timeId && state?.meta?.paths?.per_time){
      ...
      const wantKeys = ["pcatch_ensemble","pcatch_boaeddy","phab_scoring","phab_boaeddy","pops"];
      const parts = [];
...
function currentPerTimeKey(){
  ...
  if(mapKey==="pcatch") return (modelKey==="boaeddy") ? "pcatch_boaeddy" : `pcatch_${modelKey}`;
  if(mapKey==="phab") return (modelKey==="frontplus") ? "phab_frontplus" : (modelKey==="boaeddy" ? "phab_boaeddy" : "phab_scoring");
...
function setLegend(title){
  const mn = (typeof state.scaleMin==='number') ? (state.scaleMin*100).toFixed(1) : '0.0';
  const mx = (typeof state.scaleMax==='number') ? (state.scaleMax*100).toFixed(1) : '100.0';
  const el = $("legend");
  const currentStops = getStopsForMap(state?.map||'default');
  const grad = stopsToCssGradient(currentStops);
  const mid = (((Number(mn)+Number(mx))/2)||0).toFixed(1);
  el.innerHTML = `
    <div class="wrap">
      <div class="title">${title}</div>
      <div class="muted small">${mn}% → ${mx}%</div>
    </div>
    <div class="bar" style="background:${grad}"></div>
    <div class="ticks"><span>${mx}%</span><span>${mid}%</span><span>${mn}%</span></div>
  `;
}
...
async function computeAndRender(){
  ...
  async function loadLayerForTime(timeIso){
    ...
    let key = null;
    if(mapKey==="pcatch"){
      key = (modelKey==="boaeddy") ? "pcatch_boaeddy" : `pcatch_${modelKey}`;
    }else if(mapKey==="phab"){
      key = (modelKey==="frontplus") ? "phab_frontplus" : (modelKey==="boaeddy" ? "phab_boaeddy" : "phab_scoring");
    }else if(mapKey==="pops"){
      key = "pops";
